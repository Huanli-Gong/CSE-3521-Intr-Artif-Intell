...
class NaiveBayesClassifier(object):
    ...
    def train(self, training_sentences, training_labels):
        ...
        # self.prior
        # self.conditional
        # You can use any data structure you want.        # You don't have to return anything. self.conditional and self.prior will be called in def predict():
        ...
        prior = np.zeros(len(all_classes))

        for i in range(len(training_set)):
            prior[c[training_labels[i]]] += 1

            conditional[c[training_labels[i]]] += self.BOW(training_set[i])
        for j in range(len(c)):
            conditional[j] = conditional[j]/prior[j]
    ...
    def predict(self, test_sentence):
        ...
        for i in range(len(label_probability)):
            for j in range(len(self.BOW(test_sentence))):
                if self.BOW(test_sentence)[j] == 1:
                    label_probability[i - 1] += np.log(self.conditional[i][j]+1e-7)
                else:
                    label_probability[i - 1] += np.log(1 - self.conditional[i][j]+1e-7)
            label_probability[i - 1] += np.log(self.prior[i])
